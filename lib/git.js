...
// JS reimplementation because seenit's whole premise is that the analysis is
...
//
// `trim: false` is required for machine-readable output whose first field may
// begin with a space — `git status --porcelain -z` emits " M path", and trimming
// silently shifts every path by one character.
export function git(cwd, args, { stdin, raw, env, trim = true } = {}) {
  return new Promise((resolve, reject) => {
    const child = execFile(
      ...
      (err, stdout, stderr) => {
        ...
        resolve(raw || !trim ? stdout : stdout.trim())
      },
    )
    if (stdin !== undefined) {
