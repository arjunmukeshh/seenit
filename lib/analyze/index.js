...
import { condenseGraph } from './dag.js'
...
export async function analyzeCommit(repoRoot, commit, { cache, onProgress } = {}) {
  ...
  for (const entry of entries) {
    const cached = cache ? await cache.get(cacheKey(entry.sha, entry.path)) : null
    if (cached) {
  ...
  for (const entry of misses) {
    ...
    if (f) {
      facts.push(f)
      analyzed++
      if (cache) await cache.set(cacheKey(entry.sha, entry.path), f)
    }
...
export async function scoreFacts(repoRoot, facts, commit) {
  ...
  const cycles = findCycles(graphNodes)
  // The condensed DAG is stored, not just derived on demand, so the shape of
  // the architecture is itself version-controlled: `git diff` between two
  // ledger commits shows a cycle appearing the same way it shows a score
  // moving.
  const dag = condenseGraph(graphNodes, cycles)
  ...
  const payload = {
    ...
    'health.json': {
      // Stamped so a reader can tell which analyzer produced a stored score.
      // Version 1 graded duplication 0-100 on a scale since withdrawn.
      analyzerVersion: ANALYZER_VERSION,
    ...
    'graph/modules.json': { modules: extensibility.moduleMetrics, cycles },
    'graph/dag.json': dag,
  ...
  // productFiles is surfaced alongside fileCount because they can differ wildly
  // and only one of them is what the score describes. TypeScript's repository
  // analyzes 38,211 files of which 38,600-odd are test fixtures under
  // tests/baselines — every dimension is computed from the ~731 remaining, and
  // reporting only the larger number implies a breadth of evidence the score
  // does not have.
  return {
    payload,
    facts,
    health: totals.overall,
    dimensions,
    // The weights travel with the scores. Without them the UI cannot order
    // dimensions by influence or say how much any one of them counts, and it
    // silently fell back to object-literal order — which put the 0.06 dimension
    // above the 0.30 one.
    weights: WEIGHTS,
    fileCount: facts.length,
    productFiles: product.length,
...
export async function analyzeWorkspace(repoRoot, { cache } = {}) {
  ...
  const shas = await hashWorkingFiles(repoRoot, paths, { statIndex: cache?.statIndex })

  const facts = []
  const misses = []
  for (const path of paths) {
    const sha = shas.get(path)
    const cached = cache && sha ? await cache.get(cacheKey(sha, path)) : null
  ...
  for (const path of misses) {
    ...
    if (f) {
      facts.push(f)
      const sha = shas.get(path)
      if (cache && sha) await cache.set(cacheKey(sha, path), f)
    }
  }
