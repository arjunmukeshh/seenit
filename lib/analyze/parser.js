...
const SKIP_DIRS = new Set([
  ...
  // Next.js writes its build output to _next when statically exported — the
  // underscore form, distinct from the .next working directory. Missing it let
  // a 10,741-line compiled chunk into the corpus, and would put a user's own
  // build output into their health score.
  '_next', '__generated__', 'generated', 'autogen', 'gen',
])

// Also matches paths, not just filenames, so directory-embedded build output is
// caught. `_generated_` prefixes are a common convention that the previous
// `.generated.` suffix rule missed entirely.
const SKIP_FILE_RE =
  /(\.min\.(js|css)|\.bundle\.js|\.generated\.[a-z]+|\.pb\.go|_pb2\.py|\.d\.ts|_pb\.[a-z]+)$|(^|\/)_generated|(^|\/)static\/chunks\//i
...
// Covers Rust's `tests.rs` / `test.rs` module convention alongside the
// dot-suffixed forms; those were slipping through and landing in the product
// population, where a 9,954-line test module distorts the size distribution.
const TEST_RE =
  /(^|\/)(__tests__|tests?|spec|testing)(\/|$)|\.(test|spec)\.[a-z]+$|_test\.[a-z]+$|(^|\/)tests?\.[a-z]+$|(^|\/)conftest\.py$/i
