...
// KNOWN LIMITATION: this reads a `parameters` field directly off the function
// node, which holds for JS/TS/Python/Go but not for C/C++, where the parameter
// list hangs off a nested `function_declarator`. Parameter counts therefore
// read 0 for every C/C++ function. Calibration detects this — an all-zero
// distribution is treated as an extraction failure rather than a measurement,
// so no threshold is derived from it — but the metric is simply unavailable for
// those languages until the declarator is traversed.
...
export function extract(tree, source, { path, language }) {
  ...
  ;(function walk(node, inImport) {
    ...
    if (type === 'export_statement' || type === 'export_declaration') {
      acc.exports.push(...exportedNames(node))
    }
