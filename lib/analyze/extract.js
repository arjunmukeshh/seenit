...
function measureFunction(fnNode) {
  ...
  // Iterative, for the same reason findCycles is: the recursive form overflows
  // the stack on real input. TypeScript's own test suite contains
  // binderBinaryExpressionStress.js, a file built to nest binary expressions as
  // deeply as a parser will take, and it crashed the entire 38,000-file scan —
  // not just its own file. An explicit stack has no such limit.
  const work = [{ node: fnNode, nesting: 0 }]
  while (work.length) {
    const { node, nesting } = work.pop()
    ...
    if (node.isNamed && BRANCH.has(node.type)) {
      cyclomatic++
      cognitive += 1 + nesting
    }
    ...
    const isBlock = BLOCK.has(node.type)
    const deeper = isBlock ? nesting + 1 : nesting
    if (isBlock) maxNesting = Math.max(maxNesting, deeper)

    for (let i = 0; i < node.childCount; i++) {
      const c = node.child(i)
      if (c && !FUNCTION.has(c.type)) work.push({ node: c, nesting: deeper }) // nested fns measured separately
    }
  }
...
// A window has to say something. Below this many distinct token types it is a
// repeated pattern, not a structure — and after normalization a list of two
// hundred strings is literally `STR , STR , STR , …`, which is byte-identical
// to every other list of two hundred strings.
//
// This was the single largest source of false positives. On vite, the top six
// findings by shared-fingerprint count were ALL long literal lists colliding
// with each other: ROLLUP_HOOKS (hook names) against RESERVED_WORDS (JS
// keywords) against a list of spec file paths, at 2,428 shared fingerprints —
// enough to bury every real result underneath them. Six of six wrong at the top
// of the report is not noise, it is a systematic collision between things that
// are identical by construction.
const MIN_GRAM_VARIETY = 5

function fingerprint(tokens, lines) {
  if (tokens.length < K_GRAM) return []
  const grams = []
  for (let i = 0; i + K_GRAM <= tokens.length; i++) {
    const window = tokens.slice(i, i + K_GRAM)
    if (new Set(window).size < MIN_GRAM_VARIETY) continue
    grams.push([hashTokens(window), lines[i]])
  }
...
export function extract(tree, source, { path, language }) {
  ...
  // Iterative for the stack-depth reason given in measureFunction. Children are
  // pushed in reverse so they pop left-to-right: unlike the metric accumulators,
  // this walk's output IS order-sensitive — `tokens` is a sequence, and the
  // duplication fingerprints hash it directly, so visiting siblings in stack
  // order would silently change every clone report.
  const work = [{ node: root, inImport: false }]
  while (work.length) {
    const { node, inImport } = work.pop()
    const isImport = collectNode(node, acc, inImport)
    for (let i = node.childCount - 1; i >= 0; i--) {
      const c = node.child(i)
      if (c) work.push({ node: c, inImport: isImport })
    }
  }
