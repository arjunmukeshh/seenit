...
export const WEIGHTS = {
  ...
  duplication: 0.13, // reported but not graded — see duplication.js; rollup drops it
...
export const WEIGHT_PROVENANCE = {
  ...
  coverage: 'untested',
  duplication: 'counted, not graded — the 0-100 scale was uncalibrated and its denominator scaled with repo size',
  standards: 'untested',
}
...
// Neutralise scores from an older analyzer before anything displays them.
//
// Demoting a metric in the live path does not reach history. Every snapshot
// written before version 2 stored `duplication: { score: 100 }` from a scale
// that has since been withdrawn — so `seenit log`, `seenit diff` and the
// snapshot view in the UI all went on showing a perfect duplication score for a
// repository the front page said had 34 duplicate pairs. Hiding it in one place
// is not the same as fixing it.
export function normalizeStoredHealth(health, currentVersion) {
  if (!health || health.analyzerVersion === currentVersion) return health
  const duplication = health.dimensions?.duplication
  if (!duplication || typeof duplication.score !== 'number') return health
  return {
    ...health,
    dimensions: {
      ...health.dimensions,
      duplication: {
        ...duplication,
        score: null,
        reason: 'scored by an older analyzer, on a duplication scale since withdrawn',
      },
    },
    // The overall was computed WITH that score, so it is stale too. Kept rather
    // than recomputed — the raw metrics needed to redo it are not all stored —
    // but flagged so nothing presents it as current.
    stale: true,
  }
}
