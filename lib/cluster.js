...
//
// jscpd emits pairs. One file copied into six places is fifteen pairs of the
// same fact, which reads as a wall. Union-find over the block graph: files
// transitively linked by any duplicated region are one finding.
export function clusterBlocks(blocks) {
  ...
  for (const b of blocks) {
    ...
    g.files.add(b.b)
    g.pairs++
    // Anchor on the largest region — the pair worth opening first.
  ...
  return [...groups.values()]
    .map((g) => ({
      ...
      // Named, not counted: "20 more files" doesn't say which.
