...
//
// This module exists because seenit flagged it: running `review_changes`
// against its own working tree reported that bin/seenit.mjs and
...
// names imply. FORCE_COLOR exists because a pipe is not always a machine
// reading: CI logs render ANSI, and so does the screenshot pipeline that renders
// the README's terminal image from these exact bytes.
export const colorsFor = (tty, env = process.env) => {
  if (env.NO_COLOR) return PLAIN
  if (env.FORCE_COLOR && env.FORCE_COLOR !== '0') return ANSI
  return tty ? ANSI : PLAIN
}
...
// The health block shown by `seenit check` and by the MCP check_health
