...
//
// Layout: a *bare sidecar repo* at <repo>/.git/seenit/ledger.git.
//
// Why a sidecar rather than custom refs (refs/seenit/*) in the source repo:
...
// refs/seenit/* later, for a `publish` command that shares the ledger.
...
// 2: duplication stopped emitting a 0-100 score (its scale was withdrawn), and
// findings are ranked by aligned run rather than raw overlap. Snapshots written
// by version 1 carry a duplication score that no longer means anything, so the
// version is what lets a reader tell them apart.
export const ANALYZER_VERSION = 2
export const LEDGER_REL = 'seenit/ledger.git'
...
// Where the ledger lived before the tool was renamed from gitcodebase.
//
// Carried rather than dropped because the ledger IS the product's history — a
// rename that silently orphans every snapshot anyone has taken is a data loss
// bug wearing a marketing hat. One directory move, once, then never again.
const LEGACY_LEDGER_REL = 'gitcodebase/ledger.git'

export async function openLedger(repoRoot, gitDirPath) {
  const ledgerDir = join(gitDirPath, LEDGER_REL)
  const legacyDir = join(gitDirPath, LEGACY_LEDGER_REL)

  if (!existsSync(ledgerDir) && existsSync(legacyDir)) {
    await mkdir(dirname(ledgerDir), { recursive: true })
    await rename(legacyDir, ledgerDir)
    // The parent `gitcodebase/` directory is now empty; leaving it behind would
    // look like a second, broken ledger to anyone poking around in .git.
    await rm(dirname(legacyDir), { recursive: true, force: true })
  }
  ...
  if (!existsSync(ledgerDir)) {
    ...
    await ledger.run(['config', 'user.name', 'seenit'])
    await ledger.run(['config', 'user.email', 'seenit@local'])
...
//   git --git-dir=.git/seenit/ledger.git log \
