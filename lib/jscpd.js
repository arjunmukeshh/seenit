...
import { syncShadow } from './shadow.js'
 ...
 */
export async function detectNormalized(
  root,
  files,
  { minTokens = JSCPD_DEFAULT_MIN_TOKENS, includeSecondary = false, extra = [], cache = false } = {},
) {
  // The cached shadow is reused across runs; a temporary one is thrown away.
  // Planted files are removed either way, so the cache never keeps a probe.
  const shadow = cache ? (await syncShadow(root, files)).dir : await mkdtemp(join(tmpdir(), 'seenit-shadow-'))
  const planted = []
  try {
    const stats = cache ? { cached: true } : await buildShadow(root, files, shadow)
    for (const { path, source } of extra) {
      const text = await normalizeSource(path, source)
      await writeFile(join(shadow, path), text ?? source)
      planted.push(join(shadow, path))
    }
    const blocks = await detect([shadow], { minTokens, base: shadow, includeSecondary })
    return Object.assign(blocks, { shadow: stats })
  } finally {
    await Promise.all(planted.map((p) => rm(p, { force: true })))
    if (!cache) await rm(shadow, { recursive: true, force: true })
  }
}
