// jscpd wrapper: spawn the binary, normalize its JSON into blocks.
//
// Blocks carry contiguous start/end line ranges, which is what makes findings
// openable. See detectNormalized at the bottom for the path the product uses.
...
// jscpd's own default, kept. The recall study found the bar barely affects
// recall until statements are reordered, so there is nothing to gain by
// inventing a different number.
export const JSCPD_DEFAULT_MIN_TOKENS = 30

// Not the user's code. jscpd honours .gitignore, but build output is often
// committed and .git itself is always scanned otherwise — git's sample hooks
// match each other and outrank real findings.
const IGNORE = [
  '**/.git/**',
  ...
  // Data and prose. jscpd covers markdown and JSON too; here that means repeated
  // headings and two similar result files outranking real code.
  '**/*.json', '**/*.lock', '**/*.snap',
...
/**
 * Detection over normalized source. This is the path the product uses — `detect`
 * alone only finds copies nobody edited.
 *
 * Blocks come back addressed by original paths and line numbers. `extra` plants
 * files that are not in `root`, which is how find_existing compares a candidate
