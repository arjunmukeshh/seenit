...
export async function findExisting(
  root,
  snippet,
  { minTokens = JSCPD_DEFAULT_MIN_TOKENS, language = 'js', limit = 5, cache = false } = {},
) {
  ...
  //
  // The name is unique per call because the cached shadow is shared: two seenit
  // processes probing the same repository at once must not overwrite each
  // other's probe, or one gets the other's answer.
  const ext = language.startsWith('.') ? language : `.${language}`
  const probe = `__seenit_probe_${process.pid}_${Date.now().toString(36)}__${ext}`
